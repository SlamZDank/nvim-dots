return {
  {
    ...
    config = function()
      ...
      keymap("n", "<Leader>db", dap.toggle_breakpoint, { desc = "Toggle Debugger Breakpoint" })
      keymap("n", "<Leader>dcb", dap.clear_breakpoints, { desc = "Clear Debugger Breakpoints" })
      keymap("n", "<Leader>dx", dap.terminate, { desc = "Kill the Debugger" })

      keymap("n", "<Leader>dr", dap.restart, { desc = "Restart the Debugger" })
